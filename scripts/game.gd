...
func on_platform_just_exited():
	_platform_chance_logic()
	_spawn_caller()

func _spawn_caller():
	call_deferred("_spawn_platform")
	
func _platform_chance_logic():
	if special_platforms_chance == 0:
		special_platforms_chance = SPECIAL_PLATFORMS_CHANCE_INCREMENT
	elif special_platforms_chance <= SPECIAL_PLATFORMS_CHANCE_CAP:
